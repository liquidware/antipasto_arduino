...
import antipasto.app.debug.Target;
...
public class Base {
  ...
  static String VERSION_NAME = "0018-Antipasto-0042";
...
//    System.out.println("env is now " + clib.getenv("DYLD_LIBRARY_PATH"));
    ...
    //if (1==1) {
    //   throw new RuntimeException("Test Exception thrown on a thread");
    //}
...
//    }
  ...
  public Base(String[] args) {
    ...
    System.err.println("before targetsTable=");

    targetsTable = new HashMap<String, Target>();
    loadHardware(getHardwareFolder());

    System.err.println("after loadHardware");

    loadHardware(getSketchbookHardwareFolder());

     System.err.println("after loadHardware2");

    // Check if there were previously opened sketches to be restored
    boolean opened = restoreSketches();

    System.err.println("after restoreSketches");

    System.err.println("About to process " + args.length + " command line arguments");
    ...
    System.err.println("after process command line arguments");
  ...
  /**
    ...
    protected boolean restoreSketches() {
        ...
        System.err.println("inside restoreSketches");
        ...
        if (Preferences.get("last.screen.height") != null) {
            ...
            if ((screen.width != screenW) || (screen.height != screenH)) {
                ...
                System.err.println("Window position not valid because" +
                                   "Last Screen @" + screenW + "X" + screenH +
                                   "Current Screen @" + screen.width + "X" + screen.height);
            } else {
              System.err.println("Last Screen matches Current Screen");
            }
            /*
         ...
         */
            ...
            windowPositionValid = false;
            System.err.println("last.screen.height=null, last window position NOT valid");
        }
        ...
        System.err.println("Reading last.sketch.count");
        int count = 0;
        int opened = 0;

        if (Preferences.get("last.sketch.count") != null) {
           count = Preferences.getInteger("last.sketch.count");
        }

        System.err.println("Preparing to open " + count + " previous sketches");
        for (int i = 0; i < count; i++) {
            ...
            if (handleOpen(path, location) != null) {
                opened++;
                System.err.println("Sucessfully opened sketch '" + path +"'.");
            } else {
                System.err.println("Failed to open sketch '" + path +"', because file does not exist.");
            }
        }
        System.err.println("Sucessfully opened " + opened + " previous sketches");
        return(opened > 0);
    }
  ...
  */
  ...
  public void handleNew() {
    try {
      System.err.println("about to createNewUntitled");
      String path = createNewUntitled();
      System.err.println("done createNewUntitled");

      if (path != null) {
        System.err.println("path from createNewUntitled is '" + path + "'");
        System.err.println("about to handleOpen with path '" + path + "'");
        Editor editor = handleOpen(path);
        editor.untitled = true;
    } else {
        System.err.println("path from createNewUntitled is null");
    }
...
//      return false;
  ...
  protected Editor handleOpen(String path, int[] location) {
    System.err.println("inside handleOpen " + path);
...
//      }
  ...
  public void rebuildExamplesMenu(JMenu menu) {
    //System.out.println("rebuilding examples menu");
    System.out.println("rebuildExamplesMenu() ' " + examplesFolder.getAbsolutePath() + "'");
    try {
      menu.removeAll();
        ...
        AbstractAction action =
          new AbstractAction(target.getBoards().get(board).get("name")) {
            public void actionPerformed(ActionEvent actionevent) {
              //System.out.println("Switching to " + (String) getValue("target") + ":" + (String) getValue("board"));
              Preferences.set("target", (String) getValue("target"));
              Preferences.set("board", (String) getValue("board"));
              getActiveEditor().changeActiveBoard((String) getValue("board"));
            }
          };
...
//      }
  ...
  static public Map<String, String> getBoardPreferences() {
    //added error checking for people with wierd/old preferences
    if (getTarget().getBoards().get(Preferences.get("board")) == null) {
      //  error handling: Create default board preferences based on the
      //  arduino target and atmega328 board
      String board = "atmega328";
      String target = "arduino";
      Target t = Base.targetsTable.get(target);
      return t.getBoards().get(board);
    } else {
      //normal
...
//      <string>$JAVAROOT</string>
  ...
  static public Editor getActiveEditor() {
    return activeEditor;
  }

  static public Target getCurrentTarget() {
    ...
    if (core.indexOf(':') == -1) {
      ...
      t = new Target(core, coreFolder);
    } else {
      t = Base.targetsTable.get(core.substring(0, core.indexOf(':')));
  ...
  static public void setExamplesFolder(File folder) {
    examplesFolder = folder;
  }

  static public void setLibrariesFolder(File folder) {
