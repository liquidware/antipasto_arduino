...
// $Id: RF12.cpp 4885 2010-03-20 20:42:26Z jcw $
...
void rf12_initialize (uint8_t id, uint8_t band, uint8_t g) {
    ...
    rf12_xfer(0x80C7 | (band << 4)); // EL (enable TX reg), EF (enable RX FIFO), 12.0pF 
    rf12_xfer(0x820D); // EX (enable xtal), DC (disable clk pin), enable lbd
...
char rf12_easyPoll () {
    if (rf12_recvDone() && rf12_crc == 0) {
        ...
        // else if ((rf12_hdr & ~RF12_HDR_MASK) == RF12_HDR_ACK) {
        //     // ack a msg sent to me, if requested
        //     Serial.println("reply-ack");
        //     rf12_sendStart(RF12_HDR_CTL | RF12_HDR_DST | myAddr, 0, 0);
        // }
    }
    if (ezPending > 0) {
